  ...
  when (viewModel.state) {
    is HubScreenViewModel.State.Loaded -> {
      LazyVerticalGrid(contentPadding = PaddingValues(if (needContentPadding) 16.dp else 0.dp), verticalArrangement = Arrangement.spacedBy(if (needContentPadding) 8.dp else 0.dp), horizontalArrangement = Arrangement.spacedBy(if (needContentPadding) 8.dp else 0.dp), columns = GridCells.Fixed(2)) {
        ...
        (viewModel.state as HubScreenViewModel.State.Loaded).data.forEach { item ->
    ...
    is HubScreenViewModel.State.Error -> {
      Box(Modifier.fillMaxSize()) {
        ...
        Column(
          ...
          Icon(Icons.Default.Error, contentDescription = null, modifier = Modifier
            .align(Alignment.CenterHorizontally)
            .size(56.dp)
            .padding(bottom = 12.dp))
          Text("An error occurred while loading the page.", modifier = Modifier.align(Alignment.CenterHorizontally))
        }
...
class HubScreenViewModel @Inject constructor(
  ...
  suspend fun load(loader: suspend SpApiManager.() -> HubResponse) {
    _state.value = try {
      State.Loaded(spApiManager.loader().body)
    } catch (e: Exception) {
      e.printStackTrace()
      State.Error(e)
    }
  ...
  sealed class State {
    class Loaded (val data: List<HubItem>): State()
